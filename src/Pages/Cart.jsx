...
export default function UserCartPage() {
  ...
  const { cart = [], loading: cartLoading, error } = useSelector(
    (state) => state.cart
  );
  ...
  const handleAddToCart = (productItem) => {
    const existing = cart.find((i) => i.id === productItem.id);
    if (existing) {
      dispatch(updateCartQuantity({ id: existing.id, size: existing.size, quantity: existing.quantity + 1 }));
    } else {
  ...
  const handleQuantityChange = (item, change) => {
    ...
    if (newQty <= 0) {
      ...
      dispatch(updateCartQuantity({ id: item.id, size: item.size, quantity: newQty }));
    }
  };
              ...
              {cart.map((item, index) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() => dispatch(deleteCartItem({ id: item.id, size: item.size }))}

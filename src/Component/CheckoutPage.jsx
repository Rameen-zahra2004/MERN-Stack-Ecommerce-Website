...
export default function CheckoutPage() {
  ...
  const handlePaymentSuccess = (data) => {
    ...
    const order = {
      ...
      payment: {
        method: activeMethod,
        reference: data?.paymentIntentId || data?.paypalOrderId || data?.id || "N/A",
      },
      items: cart,
      totalPrice: total,
      status: "pending",
    ...
    <div className="max-w-6xl mx-auto p-6">
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="bg-white shadow rounded-lg p-6 space-y-6">
          ...
          {/* Shipping Address — identical to your original */}
          <div>
            <h2 className="font-semibold text-xl mb-3">Shipping Address</h2>
            {Object.keys(shipping).map((field) => (
              <div key={field} className="mb-2">
                <label htmlFor={`shipping-${field}`} className="block text-sm font-medium mb-1">
                  {field.charAt(0).toUpperCase() + field.slice(1)}
                </label>
                <input
                  id={`shipping-${field}`}
          ...
          <div>
            <h2 className="font-semibold text-xl mb-3">Payment Method</h2>
            ...
            <div className="grid grid-cols-2 gap-3 mb-5">
              {PAYMENT_METHODS.map((method) => (
                ...
                >
                  <span className={activeMethod === method.id ? "text-indigo-600" : "text-gray-400"}>
                    {method.icon}
                  </span>
                  <div>
            ...
            {/* Error Banner */}
            {paymentStatus === "error" && (
              <div className="mb-4 flex items-start gap-3 bg-red-50 border border-red-100 text-red-700 rounded-xl px-4 py-3 text-sm">
        ...
        {/* RIGHT: Order Summary — identical to your original */}
        <div className="bg-white shadow rounded-lg p-6 flex flex-col">
          <h2 className="font-semibold text-xl mb-4">Order Summary</h2>
